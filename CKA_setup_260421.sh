...
setup_q4() {
  ...
  info "기존 Default StorageClass 생성 (나중에 Default 해제 필요)"

  # 기존 default SC 역할을 할 더미 SC 생성
  kubectl apply -f - <<'EOF'
...
EOF
  ...
  echo -e "  ${YELLOW}※ 실습:${NC} local-kiddie SC 생성 후 standard의 default 어노테이션 제거"
}
...
EOF
  ...
  # ── Step 4: ingress-nginx ValidatingWebhook 제거 후 HTTPS Ingress 생성 ──
...
EOF
  ...
  # ── Step 5: /etc/hosts 에 gateway.web.k8s.local 등록 ──
...
setup_q9() {
  ...
  # cri-dockerd .deb 파일 다운로드 (이미 있으면 스킵)
  if [ ! -f ~/cri-dockerd_0.3.15.3-0.ubuntu-jammy_amd64.deb ]; then
    info "cri-dockerd .deb 파일 다운로드 중..."
    cd ~
    # 실제 킬러코다 환경에선 파일이 미리 존재하므로, 여기선 심볼릭 시뮬레이션
    curl -sLo ~/cri-dockerd_0.3.15.3-0.ubuntu-jammy_amd64.deb \
      "https://github.com/Mirantis/cri-dockerd/releases/download/v0.3.15/cri-dockerd_0.3.15.3-0.ubuntu-jammy_amd64.deb" 2>/dev/null || \
      warn ".deb 파일 다운로드 실패 (네트워크 제한 환경) - 파일이 이미 있어야 함"
  else
    success ".deb 파일 이미 존재"
  fi
  ...
  echo -e "  ${YELLOW}※ 실습:${NC} dpkg -i 설치 → systemctl enable --now → sysctl 설정"
}
